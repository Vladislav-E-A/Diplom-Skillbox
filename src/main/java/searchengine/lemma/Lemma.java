...
public class Lemma {
    ...
    private final LemmaConfig lemmaConfig;
    ...
    public List<String> getLemma(String word) throws IOException {
        ...
        if (checkLanguage(word).equals("Russian")) {
            List<String> baseRusForm = lemmaConfig.russianLuceneMorphology().getNormalForms(word);
    ...
    private boolean isCorrectWordForm(String word) throws IOException {
        List<String> morphForm = lemmaConfig.russianLuceneMorphology().getMorphInfo(word);
        for (String l : morphForm) {
            if (l.contains("ПРЕДЛ") || l.contains("СОЮЗ") || l.contains("МЕЖД") || l.contains("ВВОДН")
